...
export default function ClientesPage() {
  ...
  async function handleSaveBirthday(clientId: string) {
    ...
    try {
      ...
      if (res.ok) {
        setClients((prev) =>
          prev.map((c) => c.id === clientId ? { ...c, birthday: birthdayInput || null } : c)
        );
        setEditingId(null);
      }
    } catch (err) {
            ...
            return (
              ...
              >
                ...
                <div style={{ display: "flex", gap: "1.5rem", alignItems: "center" }}>
                  <div>
                    <span style={{ fontFamily: "var(--font-body)", fontSize: "0.58rem", letterSpacing: "0.15em", textTransform: "uppercase", color: "var(--rule)" }}>
                      Última visita
                    </span>
                    <p style={{ fontFamily: "var(--font-body)", fontSize: "0.78rem", color: "var(--stone)", margin: "0.1rem 0 0" }}>
                  ...
                  <div>
                    ...
                    {isEditing ? (
                      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginTop: "0.1rem" }}>
                        ...
                        />
                        <button
                          onClick={() => handleSaveBirthday(client.id)}
                          disabled={saving}
                          style={{
                            background: "var(--ink)",
                            color: "var(--cream)",
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginTop: "0.1rem" }}>
                        <p style={{ fontFamily: "var(--font-body)", fontSize: "0.78rem", color: "var(--stone)", margin: 0 }}>
                          {formatBirthday(client.birthday)}
                        </p>
                        <button
                          onClick={() => {
                            setEditingId(client.id);
                            setBirthdayInput(client.birthday ?? "");
                          }}
                          style={{
                            background: "none",
                            border: "none",
